...
![image-20211203135548833](jni.assets/image-20211203135548833.png)
...
![image-20211203140709868](jni.assets/image-20211203140709868.png)



![image-20211203140731131](jni.assets/image-20211203140731131.png)
...
3. JNI常用的数据类型及对应字符: 
   ...
   boolean 不是B 因为已经被byte占用了；long 不是L因为也被使用了
...
![image-20211203142616435](jni.assets/image-20211203142616435.png)
...
1. JNIEnv概念 :
    JNIEnv是一个**线程相关**的结构体, 该结构体代表了 Java 在本线程的运行环境。通过JNIEnv可以调用到一系列JNI系统函数。
2. JNIEnv线程相关性：
...
![image-20211203111729948](jni.assets/image-20211203111729948.png)





![image-20211203111759675](jni.assets/image-20211203111759675.png)



jobject就是java方法所在的类对象；如果方法变成了static  jobject就变成了 jclass

![image-20211203140311698](jni.assets/image-20211203140311698-16385113933372.png)



![image-20211203140344674](jni.assets/image-20211203140344674.png)
...
- add_library 指令
  ...
  libname 相当于声明了一个变量；后面就可以使用这个变量
...
//  hello是存在于C的内存区域的，要给到java的内存区域要经过转换。  c_str 返回的指针 
//  通过这个指针拿到C++内存区域数据，在newString 在java内存 创建一个相同的字符串
```
...
**按照视频的讲解，查找方法的时候是方法名和参数构成的一个整体字符串来唯一标记的。C++支持重载 会自动将将方法名加上JNIEnv *, jobject两个参数。**

**而C是没有这两个参数的。所以不加extern C 就相当于把原来的方法标记改了。导致代码执行的时候找不到方法，报错**

![image-20211203113820946](jni.assets/image-20211203113820946-16385027028441.png)

jint 代表返回值

JNICALL 空的宏；发现删除掉也不会影响方法调用



```C++
#define JNIIMPORT
//宏定义；  __attribute__ ((visibility ("default"))) 类似java的public private 可见性定义。  __attribute__属性 visibility 可见性 default默认属性  还有比如：hidden 
...
![image-20211203113854007](jni.assets/image-20211203113854007.png)

![image-20211203135503569](jni.assets/image-20211203135503569.png)
...
**可见，普通的“add”方法是直接把字节码放到“code属性表”中，而native方法，与普通的方法通过一个标志“ACC_NATIVE”区分开来。java在执行普通的方法调用的时候，可以通过找方法表，再找到相应的code属性表，最终解释执行代码，那么，对于native方法，在class文件中，并没有体现native代码在哪里，只有一个“ACC_NATIVE”的标识，那么在执行的时候改怎么找到动态链接库的代码呢？**

接着跟踪代码，只能从`System.loadLibrary()`入手了！下面是曲折的跟踪过程：

![image-20211203163019377](jni.assets/image-20211203163019377.png)

load so进来的时候会生成一个 java_包名__类名_方法名 为key，函数地址为value的表；调度执行方法的时候 就会hook libart.so 查找并执行
